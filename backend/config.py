...
class BackendConfig:
    ...
    def zone_configs(self) -> list[ZoneSafetyConfig]:
        return [
            ZoneSafetyConfig(
                zone_id=zone_id,
                cooldown_sec=int(os.getenv(f"ZONE_{zone_id.upper()}_COOLDOWN_SEC", "300")),
                max_duration_per_run_sec=int(os.getenv(f"ZONE_{zone_id.upper()}_MAX_DURATION_SEC", "30")),
                max_runs_per_hour=int(os.getenv(f"ZONE_{zone_id.upper()}_MAX_RUNS_PER_HOUR", "6")),
                max_total_water_per_day_ml=int(os.getenv(f"ZONE_{zone_id.upper()}_MAX_WATER_ML", "3000")),
                blocked=os.getenv(f"ZONE_{zone_id.upper()}_BLOCKED", "0").strip().lower() in {"1", "true", "yes", "on"},
            )
...
def load_backend_config() -> BackendConfig:
    ...
    return BackendConfig(
        ...
        database=DatabaseConfig(
            backend=os.getenv("STATE_STORE_BACKEND", "sqlite_compat").strip().lower() or "sqlite_compat",
            postgres_dsn=os.getenv("POSTGRES_DSN", "").strip(),
            sqlite_path=os.getenv("SQLITE_COMPAT_PATH", "neuroagronom.db").strip() or "neuroagronom.db",
        ),
        llama=LlamaConfig(
